...
describe("ACLs", function() {
    ...
    it("should return true for only admin if you are a module admin", function() {
        ...
        expect(app.acl.hasAccess('edit', 'Accounts')).toBeFalsy();
        expect(app.acl.hasAccess('edit', 'Accounts', {field: 'status'})).toBeFalsy();
    });
    ...
    using('provider to check hasAccessToAny method', [{
        acl: {
            'acl': {
                'Module1': {
                    'admin': 'no',
                    '_hash': '3435464127eb7ab69d413cc61'
                },
                'Module2': {
                    'admin': 'no',
                    '_hash': '3435464127eb7ab69d413cc62'
                }
            }
        },
        action: 'admin',
        expected: false,
        description: 'should return false when no "admin" access on any module'
    },{
        acl: {
            'acl': {
                'Module1': {
        ...
        action: 'admin',
        expected: true,
        description: 'should return true when there is "admin" access to Module1'
    },{
        acl: {
            'acl': {
                'Module1': {
        ...
        action: 'admin',
        expected: true,
        description: 'should return true when there is "admin" access to all modules'
    },{
        acl: {
            'acl': {
                'Module1': {
        ...
        action: 'developer',
        expected: false,
        description: 'should return false when no "developer" access on any module'
    },{
        acl: {
            'acl': {
                'Module1': {
        ...
        action: 'developer',
        expected: true,
        description: 'should return true when there is "developer" access to Module1'
    },{
        acl: {
            'acl': {
                'Module1': {
        ...
        action: 'developer',
        expected: true,
        description: 'should return true when there is "developer" access to all modules'
    }], function(value) {
        it(value.description, function() {
            app.user.set(value.acl);
            app.events.trigger('app:sync:complete');
            expect(app.acl.hasAccessToAny(value.action)).toBe(value.expected);
            expect(app.acl.hasAccessToAny(value.action)).toBe(value.expected);
        });
    });
});
