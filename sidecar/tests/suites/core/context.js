...
describe('Core.Context', function() {
    ...
    it('should prepare data for a global search', function() {
        ...
        expect((collection instanceof app.MixedBeanCollection)).toBeTruthy();
    ...
    it('should build a mixed bean collection for each one of its `collection` fields', function() {
        ...
        _.each(collectionFieldNames, function(fieldName) {
            expect(bean.get(fieldName) instanceof app.MixedBeanCollection).toBe(true);
        });
    });
    ...
    it('should log a warning when param is boolean', function() {
        var parent = app.context.getContext({module: 'Opportunities'}),
            mock = {resetLoadFlag: sinon.stub()},
            warn = sinon.collection.stub(app.logger, 'warn');

        parent.children.push(mock);
        parent.resetLoadFlag(true);
    ...
    it('should throw a warning if model is not a Bean', function() {
        ...
        var warn = sinon.collection.stub(app.logger, 'warn');
        var fetchStub = sinon.collection.stub(Backbone.Model.prototype, 'fetch');
        var context = app.context.getContext({
            model: model,
            module: module,
            modelId: 'xyz'
    ...
    describe('resetLoadFlag', function () {
        using('different data', [
            ...
            it('should reset the correct load flags based on the given options', function () {
                ...
                sinon.collection.stub(context.children[0], 'resetLoadFlag');
    ...
    describe('when fetch flag is used', function() {
        ...
        it('should fetch collection respecting fetch', function() {
            ...
            let fetchStub = sinon.collection.stub(context.get('collection'), 'fetch');
        ...
        it('should fetch model respecting fetch', function() {
            ...
            let fetchStub = sinon.collection.stub(context.get('model'), 'fetch');
        ...
        it('should accept forceFetch option and bypass internal flag', function() {
            ...
            fetchStub = sinon.collection.stub(context.get('model'), 'fetch');
