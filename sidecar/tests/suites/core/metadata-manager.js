...
describe('Core.MetadataManager', function() {
    ...
    describe('getModuleNames', function() {
        ...
        beforeEach(function() {
            ...
            backupUserDisplayModules = app.user.get('module_list');
            ...
            app.user.set('module_list', ['Contacts', 'Accounts', 'KBDocuments']); //Change order, remove Bugs and Cases

            //Remove access `create` for `Accounts`, remove `bad` access for all
            sinon.collection.stub(app.acl, 'hasAccess', function(access, module) {
                return !((access == 'create' && module == 'Accounts') || access == 'bad');
            });
        });
        afterEach(function() {
            //Restore previous data
            fixtures.metadata.modules_info = backupAllDisplayableModules;
            app.user.set('module_list', backupUserDisplayModules);
        });
        ...
        it('should return tabs modules only', function() {
            ...
            oppositeList = _.difference(oppositeList, app.user.get('module_list'));
        ...
        it('should return all modules that user has `bad` access', function() {
            expect(app.metadata.getModuleNames({access: 'bad'})).toEqual([]);
        });
    });
    ...
    it("should delegate to view-manager if has a custom view controller", function() {
        sinon.collection.spy(app.view, 'declareComponent');
        // Hack - metadata.set with a different object results in mutation of fixtures.metadata
        var originalMeta = app.utils.deepCopy(fixtures.metadata);
    ...
    it("should delegate to template.compile if meta set with custom view template", function() {
        sinon.collection.spy(app.template, 'setView');
        sinon.collection.spy(app.template, 'setLayout');
        app.metadata.set({
            modules: {
                Taxonomy: {
                    views: {
    ...
    it("should delegate to template.compile if meta set with custom field templates", function() {
        sinon.collection.spy(app.template, "setField");
        app.metadata.set({
            modules: {
                Taxonomy: {
                    fieldTemplates: {
    ...
    describe('when syncing metadata', function() {
        ...
        it('should handle invalid JSON after fetching labels data', function() {
             ...
             var result,
                 ...
                 loggerStub = sinon.stub(app.logger,'fatal');
             SugarTest.server.respondWith("GET", /.*\/rest\/v10\/metadata.*/,
                 [200, {"Content-Type": "application/json"}, JSON.stringify(meta)]);
             // Fake response for _fetchLabels's ajax call, since meta.labels is just a URL to a JSON file
             SugarTest.server.respondWith("GET", /.*\/fixtures\/labels.json.*/,
            ...
            expect(result[0].code).toEqual('sync_failed');
            expect(result[0].label).toEqual('ERR_SYNC_FAILED');
             loggerStub.restore();
             app.metadata.clearCache();
         });

        it('should not take any action when server returns 304', function() {
            var spy = sinon.collection.spy(app.metadata, 'set');
        ...
        it("should fetch only requested types", function() {
            ...
            var getMetadataStub = sinon.collection.stub(app.api, 'getMetadata');
        ...
        it("should use the default language if app.lang not yet set", function() {
            ...
            ajaxStub = sinon.collection.stub($, 'ajax'); // prevent proxying to real ajax!
            app.metadata.reset();
            ...
            expect(app.lang.getLanguage()).toEqual('en_us');
        });
        ...
        it("should include a jssource file if one is returned", function() {
            ...
            sinon.collection.spy(app.metadata, '_declareClasses');
            app.config.cacheMeta = true;
            meta.config.cacheMeta = true;
            app.metadata.reset();
            ...
            $.ajax({
                url : app.utils.buildUrl(compFixtureSrc),
                async : false,
                dataType : "script",
                success : function(){
        ...
        it("should declare classes by platform", function() {
            var compFixture = SUGAR.jssource,
                spy = sinon.collection.spy(app.metadata, '_sortAndDeclareComponents'),
                stub = sinon.collection.stub(app.view, 'declareComponent');
        ...
        describe('local storage metadata hash changes', function() {
            ...
            beforeEach(function() {
                syncMock = sinon.collection.stub(app, 'sync');
                //Set the metadata Api to just call its callback argument (index 3) and set the hash
                sinon.collection.stub(app.api, 'getMetadata', function(options) {
                    options.callbacks.success({_hash: 'bar'});
                });
                sinon.collection.stub(app, 'isServerCompatible').returns(true);
                hash = app.metadata.getHash();
                cacheStub = sinon.collection.stub(app.cache, 'get');
            });
        ...
        it("should declare module field components when field component is specified in the module metadata", function() {
            ...
            sinon.collection.stub(app.view, 'declareComponent');
    ...
    describe('deprecated getField', function () {
        beforeEach(function () {
            this.meta = {
                fields: {
                    base: {
                    ...
                    field1: {
                        templates: {
                            detail: 'field1 detail html',
                            edit: 'field1 edit html',
                        },
                    },
                },
            };
        ...
        it('should return field definitions from given type', function () {
            // there was never support for fallback, since the code isn't working
            var result = this.meta.fields.field1;
            expect(app.metadata.getField('field1')).toEqual(result);
        });
    });
