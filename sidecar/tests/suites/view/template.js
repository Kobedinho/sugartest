...
describe('template', function() {
    ...
    it('should lazy compile templates when setters called for field, view, or layout', function() {
        var addStub = sinon.stub(app.template, '_add');
        //these assertions are just here to ensure that our setters delegate to template._add (opposed to precompiling directly)
        ...
        addStub.reset();
        app.template.setView('myview', 'Cases', {}, false);
        expect(addStub).toHaveBeenCalled();
        addStub.reset();
        app.template.setLayout('mylayout', 'Cases', {}, false);
        expect(addStub).toHaveBeenCalled();
        addStub.reset();
        addStub.restore();
    });
